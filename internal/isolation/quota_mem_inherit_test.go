...
func TestRegression_AliceMemQuota_RequestExceedsOwnQuota_Denied(t *testing.T) {
	...
	_, _, err := CheckAndInjectQuota(body, quota, 1001, db, qm.GetDefaultQuota().CPUCores, qm.GetDefaultQuota().MemMB)
	if err == nil {
		t.Errorf(
...
func TestRegression_DefaultUser_MemLimit2048_Enforced(t *testing.T) {
	...
	body := []byte(fmt.Sprintf(`{"HostConfig":{"Memory":%d}}`, memBytes))
	_, _, err := CheckAndInjectQuota(body, quota, 1002, db, qm.GetDefaultQuota().CPUCores, qm.GetDefaultQuota().MemMB)
	if err == nil {
		t.Errorf(
