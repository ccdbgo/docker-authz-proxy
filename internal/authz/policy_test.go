...
func TestClassifyAction_Containers(t *testing.T) {
	tests := []struct {
		...
		{"POST", "/containers/abc123/wait", ActionWait},
		...
		{"POST", "/containers/abc123/attach", ActionAttach},
		{"POST", "/containers/abc123/resize", ActionExec},
		{"GET", "/containers/abc123/attach", ActionAttach},
		// WebSocket attach：由 pathHasPrefix+Contains("/attach") 匹配，非 pathMatchesN
		{"GET", "/containers/abc123/attach/ws", ActionAttach},
...
func TestClassifyAction_Images(t *testing.T) {
	tests := []struct {
		...
		{"GET", "/images/nginx/history", ActionHistory},
...
func TestClassifyAction_System(t *testing.T) {
	tests := []struct {
		...
		{"GET", "/version", ActionSystemVersion},
